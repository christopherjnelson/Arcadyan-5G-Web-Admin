...
| `2.4ghz.isRadioEnabled`, `5.0ghz.isRadioEnabled` | Displayed as the per-band radio state                                                | Likely understood; possibly useful | Observed as booleans in the live v2 response as well as in historical code. Issue #4 showed them `false` while per-SSID band flags were `true`, establishing that they are distinct from the v2 per-SSID band booleans. Do not change radio state without separate approval. |
...
| `2.4ghz`, `5.0ghz`: `airtimeFairness`, `isMUMIMOEnabled`, `isRadioEnabled`, `isWMMEnabled` | boolean          | `isRadioEnabled` displayed; others none | Likely radio configuration flags; read-only presence/type confirmed |
